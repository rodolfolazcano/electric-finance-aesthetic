        ...
        <>
          ...
          <div className="flex gap-1 border-b border-border/40 pb-2 mt-2">
            <button onClick={() => setTecSubTab("semaforo")}
              className={`mono text-[11px] px-3 py-1.5 rounded-md border transition-colors ${
                tecSubTab === "semaforo"
          ...
          {(semaforo.isPending || semaforoBatch.isPending) && (
            <div className="rounded-md border border-border/40 bg-background/40 px-4 py-6 text-center text-[10px] text-muted-foreground">
          ...
          {/* Error */}
          {semaforo.isError && (
            <div className="rounded-md border border-danger/30 bg-danger/5 px-4 py-3 text-[10px] text-danger">
          ...
          {semaforo.data && !semaforoBatch.data && (
            <div className="space-y-4">
              <div className="rounded-md border border-border/40 bg-background/40 p-4">
                <Suspense
                  fallback={
                    <div className="flex h-[480px] w-full items-center justify-center text-[11px] text-muted-foreground">
                      Cargando gráfico…
                    </div>
                  }
                >
          ...
          {tecMode === "portafolio-iol" && tecSubTab === "semaforo" && (
            <div className="space-y-4">
              {iol.error && (
                <div className="rounded-md border border-warning/30 bg-warning/5 px-4 py-2 text-[10px] text-warning">{iol.error}</div>
              )}
              {portError && !iol.error && (
                <div className="rounded-md border border-danger/30 bg-danger/5 px-4 py-2 text-[10px] text-danger">{portError}</div>
              )}
              ...
              {portTickers.length > 0 && (
                ...
                {iolSemaforoData && iolSemaforoData.length > 0 && (
                  <div className="rounded-lg border border-border/40 bg-background/40 overflow-hidden">
                    <div className="px-3 py-2 border-b border-border/30 text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
                      Análisis IOL
                    </div>
                    <table className="mono w-full text-[11px]">
...
function comparativoRecommendationBadge(r: "COMPRA" | "COMPRA CON CAUTELA" | "MANTENER" | "REDUCIR" | "VENTA") {
  ...
  return <span className={`rounded border px-1.5 py-0.5 text-[10px] font-mono ${m[r] ?? m.MANTENER}`}>{r}</span>;
}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
          ...
          <select
            ...
            className="h-7 rounded-md border border-border/40 bg-background/40 px-2 text-[10px] font-mono text-foreground outline-none focus:border-primary/50"
          >
            ...
            <tr className="border-b border-border/60">
              ...
              <th className="px-2 py-2 text-center">
                <button
                  ...
                  className="text-[10px] text-muted-foreground hover:text-foreground transition-colors underline decoration-dotted"
                >
            ...
            {data.map((d) => {
              ...
              >
                ...
                <td className="px-2 py-2 text-right">
                  {showFundamentals && (
                    <div className="flex gap-1 items-center justify-end text-[10px] text-muted-foreground">
        ...
        <div className="flex flex-wrap gap-3 mt-1 pl-3">
          {LEYENDA_SCORE.map((e) => (
            <span key={e.id} className="inline-flex items-center gap-1" title={e.desc}>
              ...
              <span className="text-[9px]">{e.desc}</span>
            </span>
          ))}
        </div>
      </details>
      ...
      {selectedTicker && (
        <div className="rounded-md border border-border/40 bg-background/40 p-4">
          <div className="mb-2 flex items-center justify-between">
            <span className="font-mono text-[11px] font-semibold text-primary">{selectedTicker}</span>
            <button
