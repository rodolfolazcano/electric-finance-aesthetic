...
export function analizarDistribucionRetornos(
  ...
  const cvar_95 = sortedReturns.filter(r => r <= var_95).reduce((sum, r) => sum + r, 0) / 
...
export function montoCarloSimulacion(
  ...
  const simReturns = useHistorical && historicalReturns && historicalReturns.length > 0
    ? historicalReturns
    : []
  ...
  for (let sim = 0; sim < nSimulations; sim++) {
    ...
    const returnsToUse = useHistorical && historicalReturns && historicalReturns.length > 0
      ? historicalReturns
      : generateRandomReturns(100) // fallback

    for (let day = 0; day < nDays; day++) {
      const returnIdx = useHistorical && historicalReturns && historicalReturns.length > 0
        ? day % historicalReturns.length
        : Math.floor(Math.random() * returnsToUse.length)
      const r = useHistorical && historicalReturns && historicalReturns.length > 0
  ...
  return {
    ...
    std: Math.sqrt(
      finalPrices.reduce((sum, p) => sum + Math.pow(p - mean, 2), 0) / nSimulations
    ),
 ...
 */
export function analizarHistogramaPrecios(
  prices: number[],
  periodName: string
): HistogramResult {
  ...
  for (const p of prices) {
    const binIndex = Math.min(
      Math.floor((p - minPrice) / binWidth),
      bins - 1
    )
    hist[binIndex]++
  }
...
function calcularKurtosis(data: number[]): number {
  ...
  if (m2 === 0) return 0
  const fisherKurtosis = ((n + 1) * m4) / (Math.pow(m2, 2) * (n - 1) * (n - 2) * (n - 3)) - (3 * (n - 1) * (n - 1)) / ((n - 2) * (n - 3))
...
export function calcularValorIntrinseco(
  ...
  const delta_consistente = valorIntrinseco > 0 ? tipo === 'ITM' || precio >= valorIntrinseco : true
 ...
 */
export function parsearSimboloOpcion(simbolo: string): { tipo: 'C' | 'P', strike: number, vencimiento: string } | null {
 ...
 */
export async function obtenerOpcionesIOL(
  symbol: string
): Promise<OpcionData[]> {
  try {
