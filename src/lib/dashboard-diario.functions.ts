...
import { getSector, getPaymentModality, getFrecuenciaNumerica } from "./renta-fija/ons-classification";
import { COMPLETE_ONS_WITH_FLOWS, COMPLETE_ONS_ALL_WITH_FLOWS } from "./renta-fija/ons-complete-data";
...
const BONDS_META: BondMeta[] = [
  { ticker: "BPA7D", emisor: "BCRA", vencimiento: "31/10/2027", tipoCupon: "Fixed", frecuencia: "Semiannually", isin: "AR0684877571" },
  { ticker: "BPB7D", emisor: "BCRA", vencimiento: "31/10/2027", tipoCupon: "Fixed", frecuencia: "Semiannually", isin: "AR0772251226" },
  { ticker: "BPC7D", emisor: "BCRA", vencimiento: "31/10/2027", tipoCupon: "Fixed", frecuencia: "Semiannually", isin: "AR0763285209" },
  { ticker: "BPD7D", emisor: "BCRA", vencimiento: "31/10/2027", tipoCupon: "Fixed", frecuencia: "Semiannually", isin: "AR0314171247" },
  { ticker: "BPA8D", emisor: "BCRA", vencimiento: "31/10/2028", tipoCupon: "Fixed", frecuencia: "Semiannually", isin: "AR0029227748" },
  { ticker: "BPB8D", emisor: "BCRA", vencimiento: "31/10/2028", tipoCupon: "Fixed", frecuencia: "Semiannually", isin: "AR0868821510" },
  { ticker: "GD29", emisor: "ARGENTINA", vencimiento: "9/7/2029", tipoCupon: "Fixed", frecuencia: "Semiannually", isin: "US040114HX11" },
  { ticker: "GD30", emisor: "ARGENTINA", vencimiento: "9/7/2030", tipoCupon: "Multi-coupon", frecuencia: "Semiannually", isin: "US040114HS26" },
  { ticker: "GD35", emisor: "ARGENTINA", vencimiento: "9/7/2035", tipoCupon: "Multi-coupon", frecuencia: "Semiannually", isin: "US040114HT09" },
  { ticker: "GD38", emisor: "ARGENTINA", vencimiento: "9/1/2038", tipoCupon: "Multi-coupon", frecuencia: "Semiannually", isin: "US040114HU71" },
  { ticker: "GD41", emisor: "ARGENTINA", vencimiento: "9/7/2041", tipoCupon: "Multi-coupon", frecuencia: "Semiannually", isin: "US040114HV54" },
  { ticker: "GD46", emisor: "ARGENTINA", vencimiento: "9/7/2046", tipoCupon: "Multi-coupon", frecuencia: "Semiannually", isin: "US040114HW38" },
];
...
const ON_TICKERS = Object.values(BONOS_DB)
  .filter((b) =>
    b.tipo === "ON Hard Dollar" || b.tipo === "ON CER" ||
    b.tipo === "ON Badlar" || b.tipo === "ON Tasa Fija"
  )
  .map((b) => b.ticker);
...
export const getDashboardDiario = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<DashboardRow[]> => {
    const preciosIOL = new Map<string, { precio: number; cierre: number | null; volumen: number | null }>();
      ...
      for (const ticker of tickersAPedir) {
        ...
        try {
          ...
          if (resp.ok) {
            ...
            const vol = t.volumen ?? null;
            if (precio != null && precio > 0) {
              preciosIOL.set(ticker.toUpperCase(), { precio, cierre: cierre != null && cierre > 0 ? cierre : null, volumen: vol });
            }
          }
        } catch { /* ignore */ }
      }
    ...
    function buildRow(
      ...
      const sector = bonoDb && (bonoDb.tipo === "ON Hard Dollar" || bonoDb.tipo === "ON CER" || bonoDb.tipo === "ON Badlar" || bonoDb.tipo === "ON Tasa Fija")
        ? getSector(ticker) : undefined;
    ...
    for (const meta of BONDS_META) {
      ...
      results.push(buildRow(meta.ticker, {
        emisor: meta.emisor,
        vencimiento: meta.vencimiento,
        tipoCupon: meta.tipoCupon,
        frecuencia: meta.frecuencia,
    ...
    for (const ticker of ON_TICKERS) {
      ...
      results.push(buildRow(ticker, {
        emisor: bono.descripcion?.split("-").map((s) => s.trim())[0] || bono.descripcion?.slice(0, 30) || ticker,
    ...
    if (data.tickersManual) {
      for (const ticker of data.tickersManual) {
        ...
        if (!bono) {
          results.push({
            ticker: upper, emisor: "Manual", vencimiento: "-", tipoCupon: "-",
            frecuencia: "-", tasaCupon: 0, proxPago: "-", precio: null,
            precioAnterior: null, variacion: null, yieldVal: null,
            tea: null, tna: null, modDuration: null, paridad: null,
            currentYield: null, volumen: null, moneda: "USD",
            isin: "", fuente: "estimado", tipoInstrumento: "MANUAL",
          });
          continue;
        }
        const flows = bono.flujosPorCada100VN
...
export const getOnsForLadder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<DashboardRow[]> => {
    ...
    } catch { /* live prices unavailable, use fallbacks */ }
    ...
    for (const ticker of ON_TICKERS) {
      ...
      const matchCount = bono.flujosPorCada100VN
        .filter((f) => monthSet.has(f.fecha.slice(0, 7)))
        .length;
      ...
      if (matchCount > 0) {
        candidates.push({
          ...
          row: {
            ...
            modDuration: result.durationModificada,
            paridad: result.paridad,
            currentYield: bono.cuponAnual != null && bono.cuponAnual > 0 && precio > 0 ? (bono.cuponAnual / precio) * 100 : null,
            volumen,
            moneda: bono.monedaPago ?? bono.moneda ?? "USD",
            isin: bono.isin ?? "",
