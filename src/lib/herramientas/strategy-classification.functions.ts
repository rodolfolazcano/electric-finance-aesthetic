...
export function calcularScoreCombinado(asset: StrategyAsset): number {
  ...
  if (pv >= 0) {
    earned += pv < 0.01 ? 15 : pv < 0.05 ? 10 : pv < 0.1 ? 5 : 0;
    maxPts += 15;
  }
...
function classify(
  ...
  const effectiveAlpha = pValueAlpha > 0.1 ? 0 : alpha;
  const effectiveBeta = pValueBeta > 0.1 ? 1 : beta;
  ...
  if (
    absBeta < HEDGE_FUND_BETA_MAX &&
    effectiveAlpha > HEDGE_FUND_ALPHA_MIN &&
    H >= 0.4 &&
    H <= 0.6
  ) {
  ...
  if (
    rSquared > INDEX_TRACKER_R2_MIN &&
    absBeta >= INDEX_TRACKER_BETA_RANGE.min &&
    absBeta <= INDEX_TRACKER_BETA_RANGE.max &&
    absAlpha < INDEX_TRACKER_ALPHA_MAX
  )
    return "index-tracker";

  if (
    rSquared > LONG_ONLY_R2_MIN &&
    absBeta >= LONG_ONLY_BETA_RANGE.min &&
    absBeta <= LONG_ONLY_BETA_RANGE.max &&
    effectiveAlpha > LONG_ONLY_ALPHA_MIN
  )
    return "long-only";
  ...
  if (
    rSquared > SMART_BETA_R2_MIN ||
    (betaVol != null && betaVol > SMART_BETA_BETAVOL_MIN) ||
    absBeta > SMART_BETA_BETA_ABS_MIN
  )
    return "smart-beta";
...
function computeConfidence(
  ...
  if (pValAlpha > 0.1 && pValBeta > 0.1) return "baja";
...
export const getStrategyClassification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<StrategyClassificationResult> => {
    ...
    for (const ticker of data.tickers) {
      ...
      const rollingBetas = computeRollingBetas(
        tickerMap,
        effectiveBenchmark,
        histMap,
        data.rollingBetaWindow,
      );
      const betaVol =
        rollingBetas.length > 0
          ? round4(
              Math.sqrt(
                rollingBetas.reduce(
                  (s, b) =>
                    s + (b - rollingBetas.reduce((a, b2) => a + b2, 0) / rollingBetas.length) ** 2,
                  0,
                ) / rollingBetas.length,
              ),
            )
      ...
      const returns =
        prices.length > 1 ? prices.slice(1).map((p, i) => (p - prices[i]) / prices[i]) : [];
