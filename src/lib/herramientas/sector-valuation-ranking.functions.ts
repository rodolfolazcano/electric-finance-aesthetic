...
function computePEPercentile(
  ...
  let below = 0;
  for (const v of historicalPEs) {
    if (v <= currentPE) below++;
    else break;
  }
  const pct = (below / historicalPEs.length) * 100;
  return {
    percentile: Math.round(pct * 10) / 10,
    years: Math.round((closes.length / 252) * 10) / 10,
  };
}

export const getSectorValuationRanking = createServerFn({ method: "GET" }).handler(
  async (): Promise<{
    ...
    for (const [sector, tickers] of sectorMap) {
      ...
      const mcaps = tickers.map((t) => t.marketCap).filter((v): v is number => v != null && v > 0);
      ...
      const avgPE =
        validPEs.length > 0
          ...
          : null;
      const avgFPE =
        validForwardPEs.length > 0
          ? Math.round(
              (validForwardPEs.reduce((s, v) => s + v, 0) / validForwardPEs.length) * 100,
            ) / 100
          : null;
      const avgPEG =
        validPEGs.length > 0
      ...
      const medianPct =
        sortedPcts.length > 0 ? sortedPcts[Math.floor(sortedPcts.length / 2)] : null;
      rows.push({
        sector,
        avgTrailingPE: avgPE,
        avgForwardPE: avgFPE,
