  ...
  .handler(async ({ data }) => {
    ...
    for (const item of data.items) {
      try {
        ...
        const precioUSDporAccion =
          moneda === "ARS" ? precioEnMonedaLocal / CCL_FALLBACK : precioEnMonedaLocal;
        const valorUSD =
          moneda === "ARS"
        ...
        const precioPromedioUSD =
          item.precioPromedio != null && item.precioPromedio > 0 ? item.precioPromedio : 0;
        const ultimoPrecioUSD = precioUSDporAccion;
        const plPct =
          item.precioPromedio != null && item.precioPromedio > 0
...
export const computeHedge = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<HedgeResult> => {
    ...
    const clusterMap = new Map<
      string,
      { bestFactor: string; bestAlpha: number; bestAlphaPValue: number }
    >();
    if (tickerClusters) {
      for (const c of tickerClusters) {
        clusterMap.set(c.ticker, {
          bestFactor: c.bestFactor,
          bestAlpha: c.bestAlpha,
          bestAlphaPValue: c.bestAlphaPValue,
        });
      }
    }
    ...
    const baTickers = CEDEAR_HEDGE_TICKERS.filter(
      (t) => allTickers.has(t) && !t.endsWith(".BA"),
    ).map((t) => t + ".BA");
    ...
    for (const pos of validPositions) {
      ...
      try {
        ...
        if (!posHist || posHist.length < minLen)
          throw new Error("Historial insuficiente para " + pos.ticker);

        const posPrices = alignDates({ [pos.ticker]: posHist })[pos.ticker];
        if (!posPrices || posPrices.length < 10)
          throw new Error("Datos alineados insuficientes para " + pos.ticker);
        ...
        if (benchResults.length === 0)
          throw new Error("Ningún benchmark válido para " + pos.ticker);
        ...
        if (benchResultsWithScore.length === 0)
          throw new Error("Benchmarks con R² insuficiente para " + pos.ticker);
        ...
        const equityCurve = commonDates.map((d, i) => ({
          ...
          benchmark:
            firstBench > 0 ? +((100 * bestBenchPricesArr[i]) / firstBench).toFixed(4) : 100,
        }));
        ...
        const clusterTickers =
          posClusterFactor && tickerClusters
        ...
        let hedgeCandidates = existingUniverses
          ...
          .map((t) => {
            ...
            const tipo = (
              CEDEAR_HEDGE_TICKERS.includes(t) || isCedearBA
                ? "CEDEAR"
        ...
        const scored = hedgeCandidates
          .map((h) => {
            ...
            const betaScore = Math.max(
              0,
              1 - Math.abs(h.beta - posBeta) / (Math.abs(posBeta) + 0.1),
            );
            const alphaScore = Math.max(0, 1 - Math.abs(h.beta - 1) * 0.5);
            const totalScore =
              corrScore * 0.3 + r2Score * 0.25 + betaScore * 0.25 + alphaScore * 0.2;
        ...
        const ccls = hedgeCandidates
          .map((h) => h.cclImplicito)
        ...
        const hedgeAssets = hedgeCandidates
          .map((h, i) => {
            ...
            const baHist = h.ticker.endsWith(".BA")
              ? histories[h.ticker]
              : histories[h.ticker + ".BA"];
            ...
            if (h.tipo === "CEDEAR") {
              ...
              mercadoEjecucion = "BYMA";
              const arsPrice =
                baHist?.[baHist.length - 1]?.close ?? usHist?.[usHist.length - 1]?.close;
              if (avgCCL > 0 && arsPrice && arsPrice > 0) {
                cantidadOperar = Math.round((montoUSD * avgCCL) / arsPrice);
              }
            } else if (h.tipo === "ETF") {
              mercadoEjecucion =
                h.ticker.startsWith("QQQ") || h.ticker.startsWith("TECH") ? "NASDAQ" : "NYSE";
    ...
    const ordenesConsolidadas = consolidateHedgeOrders(
      allHedgeAssets.map((a) => ({
        ticker: a.ticker,
        montoUSD: a.montoUSD,
        cantidadOperar: a.cantidadOperar,
        tipo: a.tipo,
    ...
    try {
      ...
      const uniqueHedgeTickers = [...hedgeAssetTotals.keys()].filter(
        (t) => histories[t] && histories[t].length >= 20,
      );
      if (uniqueHedgeTickers.length > 0) {
        ...
        const commonDates = firstSet
          ? [...firstSet].filter((d) => dateSets.every((s) => s.has(d))).sort()
          : [];
        ...
        if (benchmarkDates.length > 0) {
          const bmDates = [...benchmarkDates[0]].filter((d) =>
            benchmarkDates.every((s) => s.has(d)),
          );
          ...
          if (merged.length > 20) {
            ...
            const totalExposure =
              uniqueHedgeTickers.reduce((s, t) => s + Math.abs(hedgeAssetTotals.get(t) ?? 0), 0) ||
              1;
