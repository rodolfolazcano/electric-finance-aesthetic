...
async function fetchYahooHistory(
  ...
  try {
    ...
    return (rows?.quotes ?? [])
      ...
      .map((q: any) => ({
        date: new Date(q.date * 1000).toISOString().slice(0, 10),
        close: q.close,
      }));
  } catch {
...
function computeReturns(closes: { date: string; close: number }[]): number[] {
  ...
  for (let i = 1; i < closes.length; i++) {
    if (closes[i - 1].close > 0)
      rets.push((closes[i].close - closes[i - 1].close) / closes[i - 1].close);
  }
  return rets;
}
...
function buildRatio(
  ...
  for (let i = 0; i < minLen; i++) {
    if (denValues[denValues.length - minLen + i].close > 0) {
      ratios.push(
        numValues[numValues.length - minLen + i].close /
          denValues[denValues.length - minLen + i].close,
      );
    }
  }
  if (ratios.length === 0)
  ...
  const change1m =
    ratios.length > 21
      ? Math.round((ratios[ratios.length - 1] / ratios[ratios.length - 22] - 1) * 10000) / 100
      : null;
  const change3m =
    ratios.length > 63
      ? Math.round((ratios[ratios.length - 1] / ratios[ratios.length - 64] - 1) * 10000) / 100
      : null;
  ...
  return {
    ratio,
    changePct1m: change1m,
    changePct3m: change3m,
    trend,
    label,
    formula,
    interpretacion,
...
function computeTIPSSpread(
  ...
  const change1m =
    beiValues.length > 21
      ? Math.round(
          (beiValues[beiValues.length - 1] / beiValues[beiValues.length - 22] - 1) * 10000,
        ) / 100
      : null;
  const change3m =
    beiValues.length > 63
      ? Math.round(
          (beiValues[beiValues.length - 1] / beiValues[beiValues.length - 64] - 1) * 10000,
        ) / 100
      : null;
  const trend = trendFromChange(change1m);
...
export interface ReglaTransportesPetroleoResult {
  ...
  interpretacion:
    | "presion_alcista_petroleo_negativa_para_ciclo"
    | "senal_positiva_expansion_temprana"
    | "dato_no_disponible"
    | "sin_senal_clara";
...
export function reglaTransportesPetroleo(params: {
  ...
  if (
    petroleoPrices.length < 60 ||
    transportesPrices.length < 60 ||
    industrialesPrices.length < 60
  ) {
    return {
  ...
  const petroleoTrend60d =
    petroleoSlice[0] > 0
      ? ((petroleoSlice[petroleoSlice.length - 1] - petroleoSlice[0]) / petroleoSlice[0]) * 100
      : null;
  const transportesTrend60d =
    transportesSlice[0] > 0
      ? ((transportesSlice[transportesSlice.length - 1] - transportesSlice[0]) /
          transportesSlice[0]) *
        100
      : null;
  const industrialesTrend60d =
    industrialesSlice[0] > 0
      ? ((industrialesSlice[industrialesSlice.length - 1] - industrialesSlice[0]) /
          industrialesSlice[0]) *
        100
      : null;
  ...
  for (let i = 1; i < transportesSlice.length; i++) {
    if (transportesSlice[i - 1] > 0) {
      transportesReturns.push(
        (transportesSlice[i] - transportesSlice[i - 1]) / transportesSlice[i - 1],
      );
    }
  }
  ...
  if (petroleoReturns.length >= 60 && transportesReturns.length >= 60) {
    const corr = computePearsonCorrelation(
      petroleoReturns.slice(-60),
      transportesReturns.slice(-60),
    );
...
export function clasificarFaseStovall(params: {
  ...
  const sectoresConDatos = Object.keys(retornosRelativos).filter(
    (k) => retornosRelativos[k] != null,
  );
...
const STAGE_LABELS: Record<
  number,
  { label: string; description: string; leaders: string[]; avoid: string[] }
> = {
  1: {
    label: "Stage 1 — Bottom / Transición",
    description:
      "Bonos suben (flight-to-quality), Acciones caen, Commodities caen. Mercado buscando piso. Anticipar rotación hacia Technology y Consumer Cyclical.",
    leaders: ["Technology", "Consumer Cyclical"],
    avoid: ["Energy", "Basic Materials", "Financial Services"],
  },
  2: {
    label: "Stage 2 — Expansión Temprana",
    description:
      "Bonos suben (yields bajan), Acciones suben, Commodities aún débiles. Mejor momento para comprar. Lideran Technology y Consumer Cyclical.",
    leaders: ["Technology", "Consumer Cyclical", "Industrials"],
    avoid: ["Utilities", "Consumer Defensive"],
  },
  3: {
    label: "Stage 3 — Expansión Plena",
    description:
      "Los 3 activos suben sincronizados. Liquidez abundante. Technology aún fuerte, Industriales toman liderazgo.",
    leaders: ["Technology", "Industrials", "Consumer Cyclical"],
    avoid: ["Utilities", "Consumer Defensive"],
  },
  4: {
    label: "Stage 4 — Techo / Expansión Tardía",
    description:
      "Bonos caen (yields suben), Acciones suben, Commodities suben fuerte. Inflación presiona. Rotar a Energy, Basic Materials. Cautela con Technology.",
    leaders: ["Energy", "Basic Materials"],
    avoid: ["Technology", "Consumer Cyclical", "Real Estate"],
  },
  5: {
    label: "Stage 5 — Contracción Temprana",
    description:
      "Bonos caen, Acciones caen, Commodities aún suben por inercia. Inflación de costos. Refugio en Consumer Defensive y Utilities.",
    leaders: ["Consumer Defensive", "Utilities", "Healthcare"],
    avoid: ["Technology", "Consumer Cyclical", "Financial Services"],
  },
  6: {
    label: "Stage 6 — Contracción Total / Recesión",
    description:
      "Los 3 activos caen. Cash es king. Bonos largos empiezan a subir (flight-to-quality). Preparar para Stage 1.",
    leaders: ["Utilities", "Consumer Defensive", "Financial Services"],
    avoid: ["Technology", "Energy", "Basic Materials", "Consumer Cyclical"],
  },
};
  ...
  async (): Promise<IntermarketMurphyResult> => {
    ...
    try {
      ...
      copperGold = {
        ...
        label: "Copper/Gold",
        formula: "HG=F ÷ GLD",
        interpretacion: "No disponible (fallo en datos de futuros)",
        favoreceSectores: [],
      };
    }
    ...
    if (tnxData.length > 0 && fvxData.length > 0 && irxData.length > 0) {
      ...
      if (spread10y2y < 0) steepness = "inverted";
        ...
        const spread60dAgo =
          tnxData[tnxData.length - 61].close - fvxData[fvxData.length - 61].close;
      ...
      yieldCurve = {
        ...
        invertida: null,
        steepness: null,
        interpretacion: "Datos insuficientes para calcular curva de yields.",
      };
    }
    ...
    const bondsStockCorr =
      tltRets.length > 60 && spyRets.length > 60
        ? Math.round(
            (computePearsonCorrelation(tltRets.slice(-60), spyRets.slice(-60)) ?? 0) * 10000,
          ) / 10000
        : null;
    ...
    if (bondsStockCorr != null) {
      ...
      if (corrNeg && tltPos && !spyPos) {
        ...
        escenario = "AMBOS SUBEN (correlación baja)";
        trend = "incierto";
      } else if (!tltPos && !spyPos) {
        escenario = "AMBOS CAEN (correlación baja)";
        trend = "incierto";
      } else {
        escenario = "SIN SEÑAL CLARA";
        trend = "incierto";
      }
    }
    ...
    const bondsStocks: BondsStocksRelation = {
      ratio:
        tltData.length > 0 && spyData.length > 0
          ? Math.round(
              (tltData[tltData.length - 1].close / spyData[spyData.length - 1].close) * 10000,
            ) / 10000
          : null,
      trend,
      tltReturn60d: computeReturn(tltData, 60),
      spyReturn60d: computeReturn(spyData, 60),
      correlacion60d: bondsStockCorr,
      escenario,
      interpretacion:
        escenario === "DEFLACIÓN / FLIGHT-TO-QUALITY"
    ...
    const tltTrendVal =
      computeReturn(tltData, 42) != null ? computeReturn(tltData, 42)! * 100 : null; // 2-month
    const spyTrendVal =
      computeReturn(spyData, 42) != null ? computeReturn(spyData, 42)! * 100 : null;
    const crbTrendVal =
      computeReturn(crbData, 42) != null ? computeReturn(crbData, 42)! * 100 : null;
    ...
    const sectorPairs: [string, string, number[]][] = [
      ...
      [
        "XLF (Financials)",
        "Financial Services",
        computeReturns(await fetchYahooHistory("XLF", "max")),
      ],
    ...
    for (const [name, _, rets] of sectorPairs) {
      const corr60 =
        dxyRets.length > 60 && rets.length > 60
          ? Math.round(
              (computePearsonCorrelation(dxyRets.slice(-60), rets.slice(-60)) ?? 0) * 10000,
            ) / 100
          : null;
      const corr250 =
        dxyRets.length > 250 && rets.length > 250
          ? Math.round(
              (computePearsonCorrelation(dxyRets.slice(-250), rets.slice(-250)) ?? 0) * 10000,
            ) / 100
          : null;
      const interp =
      ...
      crossAssetCorrelations.push({
        pair: `DXY vs ${name}`,
        assetA: "DXY",
        assetB: name,
        correlation60d: corr60,
        correlation250d: corr250,
    ...
    if (dxyRets.length > 60 && crbRets.length > 60) {
      const ll = crossCorrelationLagged(
        dxyRets.slice(-180),
        crbRets.slice(-180),
        "DXY",
        "DBC (Commodities)",
      ...
      leadLag.push({
        pair: "DXY vs DBC",
        leader: ll.text.includes("DXY lidera")
          ? "DXY"
          : ll.text.includes("DBC lidera")
            ? "DBC"
            : "Sincrónico",
        lagDays: Math.abs(ll.bestLag),
        correlation: ll.bestCorr,
      });
    }
    // Bonds lead stocks
    if (tltRets.length > 60 && spyRets.length > 60) {
      ...
      leadLag.push({
        pair: "TLT vs SPY",
        leader: ll.text.includes("TLT lidera")
          ? "TLT (Bonds)"
          : ll.text.includes("SPY lidera")
            ? "SPY (Stocks)"
            : "Sincrónico",
        lagDays: Math.abs(ll.bestLag),
        correlation: ll.bestCorr,
    ...
    if (crbRets.length > 60 && spyRets.length > 60) {
      ...
      leadLag.push({
        pair: "DBC vs SPY",
        leader: ll.text.includes("DBC lidera")
          ? "DBC (Commodities)"
          : ll.text.includes("SPY lidera")
            ? "SPY (Stocks)"
            : "Sincrónico",
