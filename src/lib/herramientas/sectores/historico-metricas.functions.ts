...
export const fetchHistoricoValuacion = createServerFn({ method: "GET" })
  .inputValidator((d: { yfSymbol: string; rango?: "1y" | "2y" | "5y" }) =>
    z
      .object({
    ...
    try {
      ...
      if (closes.length < 10) {
        const result: HistoricoValuacionResult = {
          ...
          error:
            "Datos históricos insuficientes — no se puede calcular percentil de forma confiable",
          advertencia: null,
          fetchedAt: now,
        };
        return result;
      ...
      try {
        ...
        const raw = await fetchYahooQuoteSummaryJson<any>(data.yfSymbol, [
          "summaryDetail",
        ...
        /* fallback a derivación desde YFSnapshot */
      }
      ...
      const pbActual =
        bookValuePerShare != null && bookValuePerShare > 0
          ? precioActual / bookValuePerShare
          : null;
