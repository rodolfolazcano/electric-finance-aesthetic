...
export const PERFILES_SECTOR: Record<string, PerfilSector> = {
  Tecnología: {
    ...
    justificacion:
      "Crecimiento y márgenes pesan más que dividendos. Ciclo temprano, muy sensible a ^TNX: yields subiendo fuerte deprime valuación de flujos futuros.",
  },
  Energía: {
    fundamental: {
      correlacionCommodity: 0.4,
      capexEficiencia: 0.25,
      margenOperativo: 0.2,
      deuda: 0.15,
    },
    ...
    justificacion:
      "El precio del activo depende más de USO/petróleo que de crecimiento propio de la empresa. CAPEX intensivo, ciclo tardío.",
  },
  "Materiales Básicos": {
    fundamental: {
      correlacionCommodity: 0.35,
      capexEficiencia: 0.25,
      margenOperativo: 0.2,
      deuda: 0.2,
    },
    ...
    sensibilidadCommodity: "alta",
    justificacion:
      "Similar a Energía pero correlaciona contra una canasta (GLD/COPX/DBA) en vez de un solo commodity. Ciclo medio-tardío.",
  },
  "Servicios Financieros": {
    ...
    sensibilidadCommodity: "baja",
    justificacion:
      "Muy sensible a la pendiente de la curva (TLT/IEF/SHY) y al nivel de tasas — no al nivel absoluto de precio de un commodity. Suele anticipar el próximo ciclo de baja de tasas.",
  },
  "Consumo Cíclico": {
    fundamental: {
      crecimientoIngresos: 0.3,
      margenOperativo: 0.25,
      calidadBalance: 0.2,
      dividendos: 0.25,
    },
    ...
    sensibilidadCommodity: "baja",
    justificacion:
      "Sensible a empleo/confianza del consumidor más que a tasas o commodities directamente. Ciclo temprano-medio.",
  },
  "Defensiva del Consumidor": {
    fundamental: {
      estabilidadMargen: 0.35,
      dividendos: 0.3,
      calidadBalance: 0.2,
      crecimientoIngresos: 0.15,
    },
    tecnico: { momentum: 0.3, posicion52w: 0.3, betaBaja: 0.4 },
    sensibilidadTasas: "baja",
    sensibilidadCommodity: "media",
    justificacion:
      "Defensivo: estabilidad y dividendo pesan más que crecimiento. Ciclo de contracción temprana.",
  },
  "Acciones Industriales": {
    fundamental: {
      crecimientoIngresos: 0.25,
      capexEficiencia: 0.25,
      margenOperativo: 0.25,
      deuda: 0.25,
    },
  ...
  "Cuidado de la Salud": {
    ...
    sensibilidadCommodity: "baja",
    justificacion:
      "Defensivo con componente de crecimiento (biotech/farma). Regulatorio pesa más que macro. Ciclo de contracción temprana.",
  },
  Utilidades: {
    ...
    sensibilidadCommodity: "media",
    justificacion:
      "Proxy de bono (dividendo estable), muy sensible a tasas por descuento de flujo. Ciclo de contracción tardía.",
  },
  "Servicios de Comunicación": {
    ...
    sensibilidadCommodity: "baja",
    justificacion:
      "Mezcla de telcos defensivas (dividendo) y media/tech de crecimiento — perfil híbrido, revisar dispersión intra-sector antes de promediar.",
  },
  "Bienes Raíces": {
    ...
    sensibilidadCommodity: "baja",
    justificacion:
      "REITs: muy sensible a tasas (costo de financiamiento + descuento de flujo), poco a commodities. Solo 3 tickers en el JSON — dispersión alta, tratar con cautela estadística.",
  },
  DEFAULT: {
    fundamental: {
      crecimientoIngresos: 0.25,
    ...
    sensibilidadCommodity: "media",
    justificacion:
      "Perfil neutro — se usa SOLO si un ticker no matchea ningún sector del JSON. Debe loguearse como warning cada vez que se use.",
  },
};
...
export function buscarPerfilPorTicker(ticker: string): {
  ...
  if (!sectorEn) {
    console.warn(
      `[SectorProfile] Ticker "${ticker}" no encontrado en sectores.json — usando DEFAULT`,
    );
  ...
  if (!sectorEs || !PERFILES_SECTOR[sectorEs]) {
    console.warn(
      `[SectorProfile] Sector "${sectorEn}" (ticker=${ticker}) no tiene perfil definido — usando DEFAULT`,
    );
