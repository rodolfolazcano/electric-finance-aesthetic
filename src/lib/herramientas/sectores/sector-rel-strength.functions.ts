...
async function fetchHistoryMulti(
  tickers: string[],
...
function classifyRegime(sectors: SectorRSData[]): {
  regime: RegimeType;
  label: string;
  desc: string;
} {
  ...
  if (rising.length === 0 && falling.length === 0)
    return {
      regime: null,
      label: "Indeterminado",
      desc: "No hay datos suficientes para clasificar el régimen.",
    };
  ...
  if (growthRising >= 2) {
    return {
      regime: "growth",
      label: "Crecimiento (Growth)",
      desc: "Lideran Technology y Comunicaciones. Rotación hacia activos de crecimiento.",
    };
  }
  if (defensiveRising >= 3) {
    return {
      regime: "defensive",
      label: "Defensivo",
      desc: "Lideran Utilities, Salud y Consumo Básico. Aversión al riesgo.",
    };
  }
  if (inflationRising >= 2 && growthRising === 0) {
    return {
      regime: "inflation",
      label: "Inflación / Commodities",
      desc: "Lideran Energía y Materiales. Mercado posicionándose para inflación.",
    };
  }
  if (cyclicalRising >= 3) {
    return {
      regime: "cyclical",
      label: "Cíclico (Recuperación)",
      desc: "Lideran Financieras, Industriales y Materiales. Economía en expansión.",
    };
  }
  return {
    regime: "mixed",
    label: "Mixto / Transición",
    desc: "Señales mixtas entre sectores. Mercado sin dirección clara.",
  };
}
...
export const getSectorRelStrength = createServerFn({ method: "GET" }).handler(
  async (): Promise<SectorRelStrengthResult> => {
    ...
    const spyReturn1y =
      spyCloses.length > 1 && Number.isFinite(spyCloses[0])
