/**
 * Interpretación Sectorial Inteligente — portado de clarity-dashboard
 * (src/lib/interpretacion-sectorial.functions.ts).
 *
 * Capa de interpretación sobre la comparación sectorial. No emite
 * recomendaciones de compra/venta — describe posicionamiento relativo.
 * Cumple reglas CNV: lenguaje descriptivo, no imperativo.
 *
 * Adaptado: la interfaz ParFundamental reemplaza a FundamentalAFResult y
 * percentile viene de math/stats local.
 */

import { percentile } from "@/lib/herramientas/math/stats";
import { getEscasezPerfil } from "./escasez-taxonomia";
...
/** Campos fundamentales que consume la interpretación (por ticker y por par). */
export interface ParFundamental {
  symbol: string;
...
type ClaveMetrica = Exclude<keyof ParFundamental, "symbol" | "companyName" | "fundScore" | "error">;
...
function extraerValores(peers: ParFundamental[], key: ClaveMetrica): number[] {
  return peers
...
export function clasificarPosicionRelativa(
  ...
  if (iqr <= 0) {
    ...
    percentilAprox = (25 + (50 * (valorTicker - p25)) / iqr) as number;
  }
...
export function ajustarUmbralesPorSector(
  ...
  // Retail / Consumer: márgenes bajos por naturaleza (volumen alto)
...
const METRICAS_EVALUAR: ClaveMetrica[] = [
  "trailingPE",
  "forwardPE",
  "pegRatio",
  "priceToBook",
...
export function generarConclusionSectorialInteligente(
  ticker: ParFundamental,
  peers: ParFundamental[],
  sector: string,
  industria: string,
  ...
  for (const key of METRICAS_EVALUAR) {
    ...
    metricasAnalizadas.push({
      key,
      label: METRIC_LABELS[key as string] ?? String(key),
      valor: valorTicker,
      mediana: m,
      p25: p25v,
      p75: p75v,
  ...
  // Separar fortalezas y debilidades (lógica determinística)
  ...
  if (peersValidos.length >= 2) {
    ...
    if (candidatos.length > 0) {
      ...
      if (finalistas.length > 0) {
        ...
        const elegido =
          conPE.length > 0
            ? [...conPE].sort((a, b) => (a.pe as number) - (b.pe as number))[0]!
            : [...finalistas].sort((a, b) => b.score - a.score)[0]!;
        mejorAlternativaSector = `${elegido.ticker} (fundScore ${elegido.score}/100, P/E ${
          elegido.pe != null && elegido.pe > 0 ? elegido.pe.toFixed(1) + "x" : "N/D"
        }) — dentro de los candidatos con score comparable al líder del sector, presenta el múltiplo más bajo.`;
      }
    }
  }
