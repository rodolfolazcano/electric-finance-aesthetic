...
export function calcularRazonesPeriodo(
  ...
  const capitalTrabajo = curAssets != null && curLiab != null ? round2(curAssets - curLiab) : null;
  ...
  return {
    ...
    endDate: p.endDate,
    liquidez: {
      razonCirculante: round2(razonCirculante),
      razonRapida: round2(razonRapida),
      capitalTrabajo,
    },
    actividad: {
    ...
    endeudamiento: {
      razonDeuda: round2(razonDeuda),
      deudaPatrimonio: round2(deudaPatrimonio),
      tie: round2(tie),
    },
    rentabilidad: {
...
function interpretarLiquidez(r: RazonesPeriodo): string[] {
  ...
  if (rc != null) {
    if (rc >= 2)
      out.push(
        `Razón circulante ${rc.toFixed(2)}: cubre holgadamente el pasivo de corto plazo — posición de liquidez sólida.`,
      );
    else if (rc >= 1.5)
      out.push(`Razón circulante ${rc.toFixed(2)}: dentro del rango cómodo (1.5-2).`);
    else if (rc >= 1)
      out.push(
        `Razón circulante ${rc.toFixed(2)}: apenas cubre el pasivo corriente — vigilar la tesorería.`,
      );
    else
      out.push(
        `Razón circulante ${rc.toFixed(2)} < 1: el activo corriente no cubre el pasivo corriente — riesgo de iliquidez.`,
      );
  }
  if (rr != null) {
    if (rr >= 1)
      out.push(
        `Razón rápida ${rr.toFixed(2)}: sin depender de inventarios, se cubre el pasivo corriente.`,
      );
    else
      out.push(
        `Razón rápida ${rr.toFixed(2)} < 1: la cobertura inmediata depende de la venta de inventarios.`,
      );
  }
...
function interpretarActividad(r: RazonesPeriodo): string[] {
  ...
  if (ri != null) {
    out.push(
      `Rotación de inventarios ${ri.toFixed(2)}x: el inventario se renueva ~${ri.toFixed(1)} veces por año.`,
    );
  } else {
    out.push(
      "Rotación de inventarios N/D — el balance no reporta inventarios (común en servicios o finanzas).",
    );
  }
  if (dso != null) {
    if (dso > 90)
      out.push(
        `DSO ${dso.toFixed(0)} días: cobro lento — capital inmovilizado en cuentas por cobrar.`,
      );
    else if (dso > 45) out.push(`DSO ${dso.toFixed(0)} días: cobranza en zona normal a moderada.`);
    else
      out.push(`DSO ${dso.toFixed(0)} días: cobro rápido — buena gestión del crédito a clientes.`);
  }
  if (rf != null) {
    out.push(
      `Rotación de activos fijos ${rf.toFixed(2)}x: eficiencia en el uso de plantas/equipos para generar ventas.`,
    );
  }
  if (rt != null) {
    if (rt >= 1)
      out.push(
        `Rotación de activos totales ${rt.toFixed(2)}x ≥ 1: el activo total genera más de su valor en ventas.`,
      );
    else
      out.push(
        `Rotación de activos totales ${rt.toFixed(2)}x < 1: el activo es intensivo — comparar con el promedio industrial.`,
      );
  }
  return out;
}
...
function interpretarEndeudamiento(r: RazonesPeriodo): string[] {
  ...
  if (rd != null) {
    if (rd <= 0.5)
      out.push(
        `Razón de deuda ${(rd * 100).toFixed(1)}%: los acreedores aportan hasta la mitad del financiamiento — apalancamiento moderado.`,
      );
    else
      out.push(
        `Razón de deuda ${(rd * 100).toFixed(1)}%: más de la mitad de los activos se financia con deuda — mayor riesgo financiero (caso Allied Foods: 53.2%).`,
      );
  }
  if (dp != null) {
    out.push(
      `Deuda/Patrimonio ${dp.toFixed(2)}x: relación entre financiamiento de terceros y capital propio.`,
    );
  }
  if (tie != null) {
    if (tie >= 5)
      out.push(
        `Cobertura de intereses (TIE) ${tie.toFixed(1)}x: el EBIT cubre ~${tie.toFixed(0)} veces los intereses — holgado.`,
      );
    else if (tie >= 2) out.push(`TIE ${tie.toFixed(1)}x: cobertura aceptable aunque reducida.`);
    else
      out.push(
        `TIE ${tie.toFixed(1)}x < 2: margen muy ajustado para pagar intereses — riesgo de incumplimiento.`,
      );
  }
  return out;
}
...
function interpretarRentabilidad(r: RazonesPeriodo): string[] {
  ...
  if (m != null)
    out.push(`Margen de utilidad ${(m * 100).toFixed(1)}%: utilidad neta sobre cada $1 de ventas.`);
  if (bep != null)
    out.push(
      `BEP ${(bep * 100).toFixed(1)}%: poder de generación básica del activo, sin efecto de impuestos ni apalancamiento.`,
    );
  if (roa != null)
    out.push(`ROA ${(roa * 100).toFixed(1)}%: rendimiento de cada $1 invertido en activo total.`);
  if (roe != null) {
    if (roe * 100 >= 20)
      out.push(
        `ROE ${(roe * 100).toFixed(1)}%: rendimiento elevado sobre el capital contable (caso Allied Foods: 14.7%).`,
      );
    else if (roe * 100 > 0)
      out.push(`ROE ${(roe * 100).toFixed(1)}%: rendimiento positivo sobre el capital contable.`);
...
function interpretarMercado(r: RazonesPeriodo): string[] {
  ...
  if (pe != null) {
    if (pe <= 15)
      out.push(
        `P/U ${pe.toFixed(1)}x: precio accesible frente a utilidades — posible subvaluación.`,
      );
  ...
  if (pb != null) {
    if (pb < 1)
      out.push(`P/VL ${pb.toFixed(2)}x < 1: la acción cotiza por debajo de su valor en libros.`);
    else
      out.push(
        `P/VL ${pb.toFixed(2)}x: la prima sobre el valor en libros refleja expectativas de crecimiento.`,
      );
  }
  return out;
}
...
function interpretarDuPont(r: RazonesPeriodo): string[] {
  ...
  if (d.multiplicadorPatrimonio != null)
    partes.push(`multiplicador ${d.multiplicadorPatrimonio.toFixed(2)}x`);
  return [
...
export function calcularRazonesFinancieras(
  ...
  if (ultimo) {
    ...
    if (roe != null)
      resumen.push(`• ROE ${(roe * 100).toFixed(1)}% — retorno sobre el capital contable.`);
    if (roeD != null)
      resumen.push(
        `• DuPont: ROE descompuesto = ${roeD.toFixed(1)}% (margen × rotación × multiplicador).`,
      );
    if (ultimo.liquidez.razonCirculante != null)
      resumen.push(
        `• Razón circulante ${ultimo.liquidez.razonCirculante.toFixed(2)} — posición de corto plazo.`,
      );
    if (ultimo.endeudamiento.razonDeuda != null)
      resumen.push(
        `• Razón de deuda ${(ultimo.endeudamiento.razonDeuda * 100).toFixed(1)}% — apalancamiento.`,
      );
    if (ultimo.mercado.pe != null)
      resumen.push(`• P/U ${ultimo.mercado.pe.toFixed(1)}x — valoración de mercado.`);
  } else {
  ...
  return {
    ...
    interpretaciones: {
      ...interpretaciones,
      rentabilidad: [
        ...interpretaciones.rentabilidad,
        ...(ultimo ? interpretarDuPont(ultimo) : []),
      ],
    },
    resumen,
    error:
      historico.length === 0
        ? "No se encontraron estados financieros históricos para este símbolo."
        : null,
  };
}
