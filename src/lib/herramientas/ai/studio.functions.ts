...
export async function agentTurn(input: any): Promise<{
  ok: boolean;
  text: string;
  provider?: string;
  model?: string;
 ...
 */
export async function routeAiTask(query: string, context?: Record<string, any>) {
  try {
 ...
 */
export async function cascadeInterpretQuery(userQuery: string, context?: Record<string, any>) {
 ...
 */
export async function analyzeDataUI(input: DataInput): Promise<AnalysisResult> {
  try {
