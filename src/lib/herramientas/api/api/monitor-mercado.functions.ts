...
import {
  iolCotizarMultiples,
  iolFuturosOperables,
  type IOLCotizacionSimple,
} from "./iol-cotizaciones";
...
const BONARES_GLOBALES_TICKERS = [
  "AO27D",
  "AL29D",
  "GD29D",
  "AO28D",
...
export const getMonitorMercadoData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MonitorMercadoData> => {
    ...
    try {
      lecapPrecios = await iolCotizarMultiples({
        data: { bearerToken: token ?? undefined, simbolos: LECAP_TICKERS },
      });
    } catch {
      /* ignore */
    }
    ...
    async function calcBonos(tickers: string[], tipoLabel: string): Promise<BonoRow[]> {
      ...
      try {
        precios = await iolCotizarMultiples({
          data: { bearerToken: token ?? undefined, simbolos: tickers },
        });
      } catch {
        /* ignore */
      }
    ...
    try {
      const accPrecios = await iolCotizarMultiples({
        data: { bearerToken: token ?? undefined, simbolos: PANEL_LIDER_TICKERS },
      });
    ...
    return {
      ...
      riesgoPais:
        riesgoPais.status === "fulfilled"
          ? riesgoPais.value
          : { valor: null, variacion: null, variacionPorcentual: null, fecha: null },
      reservas,
      tamarTna,
      lecaps,
      tamar: tamarBonos,
