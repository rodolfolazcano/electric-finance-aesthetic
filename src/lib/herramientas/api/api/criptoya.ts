...
export type CasaDolar =
  "mayorista" | "oficial" | "ahorro" | "tarjeta" | "blue" | "cripto" | "mep" | "ccl";
...
async function fetchADHistorial(casa: string): Promise<ADCotizacion[]> {
  ...
  if (!endpoint) return [];
  try {
    const r = await fetch(`https://api.argentinadatos.com/v1/cotizaciones/dolares/${endpoint}`, {
      cache: "no-store",
