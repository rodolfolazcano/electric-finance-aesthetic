...
function zonaDesdePercentil(pct: number | null): {
  ...
  if (pct >= 75) return { zona: "en la parte más cara de su propio rango histórico", tono: "negativo" };
  if (pct >= 60) return { zona: "en la zona media-alta de su rango histórico", tono: "negativo" };
  if (pct <= 25) return { zona: "en la parte más barata de su propio rango histórico", tono: "positivo" };
...
export function interpretarPE(
  ...
  if (peActual == null) {
    return {
      resumen: "Esta empresa no tiene ganancias positivas en este momento, por eso no se puede calcular P/E.",
      contexto: "El P/E mide cuántas veces las ganancias anuales paga el mercado por la acción. Sin ganancias, el indicador no aplica.",
      tono: "sin-dato",
      confiabilidad: "alta",
    };
  }
  if (percentilHistorico == null) {
    return {
      ...
      contexto: "No hay suficiente historial de precios de este activo para saber si este nivel es alto o bajo para él en particular.",
      tono: "neutral",
      confiabilidad: "baja",
    };
  }
...
export function interpretarPB(
  ...
  if (pbActual == null) {
    return {
      resumen: "No hay información disponible sobre el valor contable de esta empresa (Book Value).",
      contexto: "El P/B compara el precio de la acción con el valor contable de la empresa (activos menos deudas). Sin este dato, no se puede evaluar.",
      tono: "sin-dato",
      confiabilidad: "alta",
    };
  }
  if (percentilHistorico == null) {
    return {
      ...
      contexto: "No hay suficiente historial para saber si este precio sobre valor libro es alto o bajo comparado con su propio pasado.",
      tono: "neutral",
      confiabilidad: "baja",
    };
  }
...
export function interpretarROE(
  roe: number | null,
  nombreActivo: string,
): Interpretacion {
  if (roe == null) {
    return {
      resumen: "No hay datos suficientes para calcular la rentabilidad sobre patrimonio de esta empresa.",
      contexto: "",
      tono: "sin-dato",
      confiabilidad: "alta",
    };
  ...
  if (roe > 0.2) {
    ...
    contexto = "Comparado con el promedio del mercado, un ROE superior al 20% suele indicar que la empresa usa eficientemente el capital de sus dueños para generar ganancias.";
  } else if (roe > 0.12) {
    resumen = `${nombreActivo} genera ${roePct}% de ganancia anual sobre el capital invertido — un nivel saludable.`;
    contexto = "Un ROE entre 12% y 20% está dentro de lo considerado bueno para empresas establecidas.";
  } else if (roe > 0.05) {
    resumen = `${nombreActivo} genera ${roePct}% anual sobre el capital — por debajo del promedio deseable.`;
    contexto = "Comparado con empresas del mismo perfil, este ROE sugiere que la rentabilidad sobre el capital es modesta.";
  } else if (roe > 0) {
    ...
    contexto = "Un ROE positivo pero bajo puede indicar que la empresa retiene mucho capital sin generar retornos proporcionales.";
  } else {
    ...
    contexto = "Esto significa que la empresa destruye valor: las ganancias no alcanzan para cubrir el costo del capital que los accionistas pusieron.";
  }
  return { resumen, contexto, tono: roe > 0.12 ? "positivo" : roe > 0.05 ? "neutral" : roe > 0 ? "negativo" : "negativo", confiabilidad: "alta" };
}

//  Interpretación de FCF Yield 

export function interpretarFCFYield(
  ...
  if (fcfYield == null) {
    return {
      ...
      contexto: "El FCF Yield mide qué porcentaje del precio de la acción la empresa genera en efectivo libre cada año. Sin este dato, no se puede evaluar su capacidad de generar efectivo real.",
      tono: "sin-dato",
      confiabilidad: "alta",
    };
  }
  ...
  return {
    resumen,
    contexto: "El FCF Yield se interpreta parecido a un rendimiento: un número más alto significa que la empresa produce más efectivo en relación a lo que cuesta. Es señal de salud financiera cuando es positivo y sostenido.",
    tono: fcfYield > 0.06 ? "positivo" : fcfYield > 0.03 ? "neutral" : fcfYield > 0 ? "negativo" : "negativo",
    confiabilidad: "alta",
  };
}
...
export function interpretarDE(
  ...
  return {
    resumen,
    contexto: "La relación deuda/patrimonio compara lo que la empresa debe (deuda total) con lo que los accionistas han puesto (patrimonio neto). Una cifra más baja implica menor apalancamiento y menos riesgo de insolvencia.",
    tono,
    confiabilidad: "alta",
  };
}
...
export function interpretarUpside(
  ...
  return {
    ...
    contexto: "Esto es el consenso de los analistas que cubren la acción. No es una garantía de precio futuro — es una estimación basada en modelos propios de cada analista. Cuantos más analistas, más representativo suele ser el promedio.",
...
export function interpretarBeta(
  ...
  if (beta == null) {
    return {
      resumen: "No hay suficiente historial de precios para medir la volatilidad de esta acción contra el mercado.",
      contexto: "",
      tono: "sin-dato",
      confiabilidad: "alta",
    };
  ...
  return {
    resumen,
    contexto: "Beta mide la sensibilidad histórica de la acción frente al mercado. Una beta mayor a 1 implica más volatilidad (más riesgo pero también más potencial de retorno). Una beta menor a 1 implica menor volatilidad. No es buena ni mala per se — depende del perfil de cada inversor.",
    tono: "neutral",
    confiabilidad: "alta",
  };
}
...
export function interpretarScore(
  ...
  return {
    resumen,
    contexto: "El score combina ROE, crecimiento de ingresos, margen neto, FCF Yield, upside de analistas y crecimiento de ganancias. Cada métrica se puntúa y se pondera para dar un resultado de 0 a 100. Un score alto sugiere métricas fundamentalmente sólidas en conjunto.",
    tono,
    confiabilidad: "alta",
  };
}
