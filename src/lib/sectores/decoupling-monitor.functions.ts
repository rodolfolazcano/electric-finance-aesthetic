    ...
    if (copxData.length > 20 && gldData.length > 20) {
      ...
      rotationSignals.push({
        ...
        interpretacion: cgTrend === "up"
          ? "🟢 Cobre > Oro: expansión industrial, demanda real. Dr. Copper confirma crecimiento."
          : cgTrend === "down"
          ? "🔴 Oro > Cobre: incertidumbre, contracción industrial anticipada. Adelanta recesión 3-6m."
          : "⚪ Sin tendencia clara.",
      });
    }
    ...
    if (rspData.length > 20 && spyData.length > 20) {
      ...
      rotationSignals.push({
        ...
        interpretacion: rsTrend === "up"
          ? "🟢 RSP > SPY: participación AMPLIA. Mercado saludable, líderes amplios."
          : rsTrend === "down"
          ? "🔴 SPY > RSP: mercado ANGOSTO. Solo mega-caps suben — señal de late cycle."
          : "⚪ Neutral.",
      });
    }
    ...
    if (xlkData.length > 20 && xleData.length > 20) {
      ...
      rotationSignals.push({
        ...
        interpretacion: teTrend === "up"
          ? "🟢 Tech > Energy: early/mid cycle. Innovación lidera."
          : teTrend === "down"
          ? "🔴 Energy > Tech: LATE CYCLE. Rotación clásica a inflación."
          : "⚪ Neutral.",
      });
    }
    ...
    if (xlyData.length > 20 && xliData.length > 20 && xlpData.length > 20 && xluData.length > 20) {
      ...
      rotationSignals.push({
        ...
        interpretacion: cdTrend === "up"
          ? "🟢 Cíclicos > Defensivas: risk-on. Expansión."
          : cdTrend === "down"
          ? "🔴 Defensivas > Cíclicos: flight to safety. Contracción."
          : "⚪ Neutral.",
      });
    }
    ...
    if (tltData.length > 20 && spyData.length > 20) {
      ...
      rotationSignals.push({
        ...
        interpretacion: bsTrend === "up"
          ? "🟢 Bonos > Stocks: flight to quality. Riesgo-off."
          : bsTrend === "down"
          ? "🟢 Stocks > Bonos: risk-on. Expansión."
          : "⚪ Neutral.",
      });
    }
    ...
    if (dxyData.length > 20) {
      ...
      rotationSignals.push({
        ...
        interpretacion: dxyTrend === "up"
          ...
          : dxyTrend === "down"
          ? "🟡 DXY cayendo: dólar débil → commodities suben, inflación importada, emergentes favorecidos. Bonos largos presionados (Murphy pp. 145-154)."
    ...
    if (eemData.length > 20) {
      ...
      rotationSignals.push({
        ...
        interpretacion: eemTrend === "up"
          ? "🟢 Emergentes subiendo: dólar débil + commodities fuertes = flujo a EM (Murphy p. 235). Confirma apetito por riesgo global."
          : eemTrend === "down"
          ? "🔴 Emergentes cayendo: dólar fuerte + commodities débiles = salida de EM. Señal de contracción global (Murphy pp. 236-237)."
