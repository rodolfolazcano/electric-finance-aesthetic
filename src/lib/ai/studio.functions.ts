 ...
 * Transcribe texto, tablas y series numéricas de imágenes financieras.
 * Input esperado: { data: { base64: string, mime: string, conversationId?: string } }
 * Retorna: { ok: true, text: string, provider, model } para que Studio lo guarde como File.text
 */
export async function readImageFile(input?: any): Promise<{ ok: boolean; text?: string; provider?: string; model?: string; error?: string }> {
  ...
  // Validar que sea base64 real (evita payloads vacíos)
  ...
  try {
    ...
    // Fallback directo a resilientVision si describeImage falla por import circular
    try {
