...
export function xirrConvencion(
  ...
  const intervalos = [
    [0.0001, 0.5], [0.5, 1.0], [1.0, 2.0], [2.0, 4.0], [-0.5, 0.0001],
    [0.0001, 1.0], [-0.3, 0.3], [-0.9, -0.3], [4.0, 8.0],
  ];
...
export function convexity(
  ...
  const denom = convention === "STREET"
    ? Math.pow(1 + tir / freq, 2)
    : Math.pow(1 + tir, 2);
...
export function precioTecnico(bono?: BonoConfig): number {
  if (bono?.valorResidualActual != null && bono.valorResidualActual > 0) return bono.valorResidualActual;
  return 100;
}
...
export async function fetchBadlarRate(): Promise<number | null> {
  try {
    const res = await fetch("https://api.bcra.gob.ar/estadisticas/v4.0/Monetarias/7", { cache: "no-store" });
    if (res.ok) {
...
export async function fetchTamarRate(): Promise<number | null> {
  try {
    const res = await fetch("https://api.bcra.gob.ar/estadisticas/v4.0/Monetarias/13", { cache: "no-store" });
    if (res.ok) {
      const j = await res.json();
      const results: Array<{ valor: number }> = j?.results ?? [];
      if (results.length > 0) return results[0].valor;
...
export function proyectarTasaFlotante(
  ...
  const yf = yearFraction(new Date(), new Date(Date.now() + diasPeriodo * 86400000), convencionDias);
  // Convertir TNA a tasa del período si es necesario
  const tasaPeriodo = tasaReferenciaTipo === "TNA"
...
export async function fetchUvaHistory(): Promise<UvaEntry[]> {
  ...
  try {
    const r = await fetch("https://api.argentinadatos.com/v1/finanzas/indices/uva", { cache: "no-store" });
    if (r.ok) {
...
export async function fetchInflacionMensual(): Promise<number | null> {
  try {
    const r = await fetch("https://api.argentinadatos.com/v1/finanzas/indices/inflacion", { cache: "no-store" });
    if (r.ok) {
...
export const getBonoPrecioYTCOficial = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const precios: Record<
      string,
      { precio: number | null; fuente: string }
    > = {};
            ...
            try {
              ...
              if (r.ok) {
                ...
                if (p != null && p > 0) {
                  ...
                  precios[ticker] = { precio: p / escala, fuente: suffix ? `iol-${suffix}` : "iol-plano" };
                  return;
                }
              }
            } catch { /* fall through */ }
...
export const calcularRendimientosBono = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    let tipo: TipoBono = bono?.tipo ?? "Hard Dollar";
    let descripcion = bono?.descripcion ?? "";
    let monedaFlujos: "USD" | "ARS" = bono?.monedaFlujos ?? "USD";
    ...
    if (bono?.ajuste === "DolarOficial" && flujosFuturos.length > 0) {
      try {
        ...
        const [al30, al30d] = await Promise.all([
          fetch(`${AD}/v1/cotizaciones/dolares/bolsa`, { cache: "no-store" }).then(r => r.ok ? r.json() : []),
        ...
        if (Array.isArray(al30) && al30.length > 0) {
          ...
          const oficialRes = await fetch(`${AD}/v1/cotizaciones/dolares/oficial`, { cache: "no-store" });
          if (oficialRes.ok) {
            const oficialArr = await oficialRes.json();
            const oficialActual = Array.isArray(oficialArr) && oficialArr.length > 0
              ? (oficialArr[oficialArr.length - 1]?.venta ?? oficialArr[oficialArr.length - 1]?.compra)
              : null;
    ...
    if ((bono?.ajuste === "BADLAR" || bono?.ajuste === "TAMAR") && flujosFuturos.length > 0) {
      try {
        const tasaReferencia = bono.ajuste === "BADLAR"
          ? await fetchBadlarRate()
          : await fetchTamarRate();
        if (tasaReferencia != null && tasaReferencia > 0) {
          // Convertir TNA a decimal
          const tasaDecimal = tasaReferencia / 100;
          flujosFuturos = flujosFuturos.map((f) => {
            const diasPeriodo = Math.round(yearFraction(
              new Date(),
              parseISO(f.fecha),
              convencionDias,
    ...
    const dModificada = tirCalc !== null
      ? durationModificadaConvencion(dMacaulay, tirCalc, freq, yieldConv)
      : null;
    const convexityVal = tirCalc !== null
      ? convexity(flujosXIRR, precioDirty, tirCalc, freq, yieldConv)
      : null;

    // Armar tabla de flujos detallada
    const diasAlVto = Math.round(yearFraction(fechaLiq, parseISO(vencimiento), convencionDias) * 365);
...
export const calcularMonitorBonos = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const iolFetchOne = async (ticker: string, token: string): Promise<{ precio: number; moneda: string } | null> => {
      try {
        const res = await fetch(`${IOL}/api/v2/BCBA/Titulos/${ticker}/Cotizacion`, {
        ...
        await Promise.allSettled(
          batch.map(async (ticker) => {
            ...
            for (const c of candidates) {
              ...
              if (result) {
                const esUsd = result.moneda.toUpperCase().includes("USD") || result.moneda.toUpperCase().includes("DOLAR");
                const suffix = c.replace(ticker, "");
    ...
    for (const ticker of tickers) {
      ...
      if (iolEntry != null && iolEntry.precio > 0) {
        ...
        const esUSD = monedaIOL.includes("USD") || monedaIOL.includes("DOLAR") || monedaIOL.includes("U$S") || monedaIOL.includes("US$");
        ...
        try {
          ...
          rb = {
            ...
            diasAlVencimiento: bonoCfg?.vencimiento
              ? Math.max(0, Math.round(diasEntre(new Date(), new Date(bonoCfg.vencimiento + "T00:00:00"))))
              : 0,
            volumenNominal: null,
            montoOperado: null,
            fuente: "Teórico",
...
export const getHistoricoParidadON = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (percentil <= 25) senal = "COMPRA";      // Paridad baja historica → compra
    else if (percentil >= 75) senal = "VENTA";  // Paridad alta historica → venta
    else senal = "MANTENER";
...
export function simularFlujoReinversion(
  ...
  if (!bono) return { ticker, error: `Bono ${ticker} no encontrado`, cuponesCobrados: 0, fechaCompra, fechaVenta, precioCompra: null, precioVenta: null, tirCompra: null, tirVenta: null, retornoTotalPeriodo: null, retornoAnualizado: null, flujos: [] };

  const fechaCompraDate = parseISO(fechaCompra);
  const fechaVentaDate = parseISO(fechaVenta);

  if (fechaVentaDate <= fechaCompraDate) {
    return { ticker, error: "fechaVenta debe ser posterior a fechaCompra", cuponesCobrados: 0, fechaCompra, fechaVenta, precioCompra: null, precioVenta: null, tirCompra: null, tirVenta: null, retornoTotalPeriodo: null, retornoAnualizado: null, flujos: [] };
  }
  ...
  try {
    ...
    if (!precioCompra) {
      const flujosEnCompra = bono.flujosPorCada100VN.filter(f => parseISO(f.fecha) > fechaCompraDate);
      if (flujosEnCompra.length > 0) {
        const ultimoCupon = interesesCorridos(fechaCompraDate, parseISO(bono.vencimiento), bono.cuponAnual ?? 0, freq, convencionDias);
    ...
    if (!precioCompra || precioCompra <= 0) {
      return { ticker, fechaCompra, fechaVenta, error: "No se pudo determinar precio de compra", cuponesCobrados: 0, precioCompra: null, precioVenta: null, tirCompra: null, tirVenta: null, retornoTotalPeriodo: null, retornoAnualizado: null, flujos: [] };
    }
    ...
    if (!precioVenta) {
      const flujosEnVenta = bono.flujosPorCada100VN.filter(f => parseISO(f.fecha) > fechaVentaDate);
      if (flujosEnVenta.length > 0) {
        precioVenta = bono.valorResidualActual ?? 100;
        const ultimoCuponVenta = interesesCorridos(fechaVentaDate, parseISO(bono.vencimiento), bono.cuponAnual ?? 0, freq, convencionDias);
        const flujosXIRR: Array<{ yf: number; monto: number }> = [
    ...
    if (!precioVenta || precioVenta <= 0) {
      return { ticker, fechaCompra, fechaVenta, error: "No se pudo determinar precio de venta", cuponesCobrados: 0, precioCompra, precioVenta: null, tirCompra, tirVenta: null, retornoTotalPeriodo: null, retornoAnualizado: null, flujos: [] };
    }
    ...
    const intCorridosCompra = interesesCorridos(fechaCompraDate, parseISO(bono.vencimiento), bono.cuponAnual ?? 0, freq, convencionDias);
    ...
    const intCorridosVenta = interesesCorridos(fechaVentaDate, parseISO(bono.vencimiento), bono.cuponAnual ?? 0, freq, convencionDias);
    ...
    const retornoAnualizado = retornoTotalPeriodo !== null && years > 0
    ...
    return { ticker, fechaCompra, fechaVenta, error: (e as Error).message, cuponesCobrados: 0, precioCompra: null, precioVenta: null, tirCompra: null, tirVenta: null, retornoTotalPeriodo: null, retornoAnualizado: null, flujos: [] };
  }
}
