...
import { getDecouplingMonitor, type DecouplingSignal, type DecouplingLevel, type CompositeScore, type YieldCurveData } from "@/lib/sectores/decoupling-monitor.functions";

// ─── Helpers ──────────────────────────────────────────────────────────────

const NIVEL_CONFIG: Record<DecouplingLevel, { label: string; color: string; bg: string; border: string; icon: string }> = {
  bajo:     { label: "Bajo",     color: "text-green-400",  bg: "bg-green-500/10",   border: "border-green-500/30",   icon: "" },
  moderado: { label: "Moderado", color: "text-yellow-400", bg: "bg-yellow-500/10",  border: "border-yellow-500/30",  icon: "" },
  alto:     { label: "Alto",     color: "text-orange-400", bg: "bg-orange-500/10",  border: "border-orange-500/30",  icon: "" },
  critico:  { label: "Crítico",  color: "text-red-400",    bg: "bg-red-500/10",     border: "border-red-500/30",     icon: "" },
};
  ...
  return (
    <Card className={cn("border-2", cfg.border, cfg.bg)}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-muted-foreground/80 mt-0.5">
              Score compuesto: {pct}% &mdash; cruzando 4 señales (correlación bonos/acciones, CRB/Bonds, curva de rendimientos, consumo defensivo vs cíclico)
            </p>
          </div>
        </div>
      </div>
      <div className="relative h-3 mx-4 mb-4 rounded-full bg-border/30">
        ...
        />
        {[25, 50, 75].map((t) => (
          <div key={t} className="absolute top-0 h-full w-px bg-border/50" style={{ left: `${t}%` }} />
        ))}
      </div>
    </Card>
  );
    ...
    <Card className="border-border/40 bg-background/40/40 p-3.5">
      ...
      <div className="grid w-full grid-cols-2 gap-2 mb-2">
        <div>
          ...
          <span className={cn("text-[11px] font-mono", signal.valor != null ? "text-foreground" : "text-muted-foreground")}>
            {fmtNum(signal.valor)}
          </span>
        </div>
        <div>
      ...
      <span className="text-[7px] font-mono text-muted-foreground/40 block mt-1">{signal.fuente}</span>
    </Card>
  );
}
    ...
    <Card className="border-border/40 bg-background/40/40 p-3.5">
      ...
      <div className="grid w-full grid-cols-4 gap-2 mb-2">
        <div>
          ...
          <span className="text-[11px] font-mono text-foreground">{yieldCurve.ust10y != null ? `${yieldCurve.ust10y.toFixed(2)}%` : "\u2014"}</span>
        </div>
        <div>
          <span className="text-[8px] font-mono text-muted-foreground block">5Y (^FVX)</span>
          <span className="text-[11px] font-mono text-foreground">{yieldCurve.ust5y != null ? `${yieldCurve.ust5y.toFixed(2)}%` : "\u2014"}</span>
        </div>
        <div>
          <span className="text-[8px] font-mono text-muted-foreground block">3M (^IRX)</span>
          <span className="text-[11px] font-mono text-foreground">{yieldCurve.irx3m != null ? `${yieldCurve.irx3m.toFixed(2)}%` : "\u2014"}</span>
        </div>
        <div>
          <span className="text-[8px] font-mono text-muted-foreground block">Spread 10Y-2Y</span>
          <span className={cn("text-[11px] font-mono", yieldCurve.invertida ? "text-red-400" : "text-green-400")}>
            {yieldCurve.spread10y2y != null ? `${yieldCurve.spread10y2y.toFixed(2)}%` : "\u2014"}
          </span>
        </div>
      </div>
      <p className="text-[9px] text-muted-foreground/70 leading-relaxed">{yieldCurve.senal.detalle}</p>
    </Card>
  );
}
...
function ResumenTable({ correlacion, crb, cons }: { correlacion: DecouplingSignal; crb: DecouplingSignal; cons: DecouplingSignal }) {
  const rows = [
    { var: "Correlación SPY vs TLT",    api: "getBenchmarksMatrix",           behavior: "Negativa (Bonds ↑ / Stocks ↓)",                   actual: `${fmtNum(correlacion.valor)}`, nivel: correlacion.nivel },
    { var: "Precio de Commodities",     api: "getYahooQuoteServer",            behavior: "Tendencia Bajista (Cobre/Oro)",                   actual: `${fmtPct(crb.valor)}`, nivel: crb.nivel },
    { var: "Yield 10Y Treasury",        api: "getSerieHistoricaConTIR",        behavior: "Mínimos históricos",                              actual: "\u2014", nivel: "bajo" as DecouplingLevel },
    { var: "Ratio Staples / Cyclicals", api: "SectoresPage",                   behavior: "STAPLES lideran (inversores buscan defensivos)",  actual: `${fmtNum(cons.valor)}`, nivel: cons.nivel },
  ];
  ...
  return (
    <Card className="border-border/40 bg-background/40/40 p-3.5">
              ...
              return (
                <tr key={r.var} className="border-b border-border/10 hover:bg-background/20/20">
                  ...
                  <td className="py-1.5 text-right">
                    <span className={cn("rounded px-1 py-0.5 text-[8px]", nc.color, nc.bg)}>{nc.label}</span>
                  </td>
                </tr>
              );
            })}
      ...
      <p className="text-[8px] text-muted-foreground/50 mt-2 leading-relaxed">
        Conclusión: Las APIs no darán un mensaje "estamos en deflación", pero al cruzar bonos al alza con commodities a la baja mediante las Server Functions, el dashboard muestra visualmente el régimen macroeconómico actual según Murphy.
      </p>
    </Card>
  );
}
