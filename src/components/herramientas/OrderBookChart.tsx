  ...
  return (
    <div className="font-mono">
      ...
      <div className="space-y-0.5 mb-2">
        <div className="text-[13px] uppercase tracking-wider text-red-400/60 mb-1">
          Ofertas (Ask)
        </div>
        {allLevels.map((a, i) => (
          <div key={`a${i}`} className="flex items-center gap-1.5 h-4">
            <div className="w-[80px] text-right text-[13px] text-red-400/80 truncate shrink-0">
              {fmt(a.price)}
            </div>
            ...
            <div className="w-[48px] text-left text-[13px] text-red-400/50 shrink-0">
              {fmt(a.volume, 4)}
            </div>
          </div>
        ))}
      ...
      <div className="space-y-0.5 mt-2">
        <div className="text-[13px] uppercase tracking-wider text-green-400/60 mb-1">
          Demandas (Bid)
        </div>
        {bidLevels.map((b, i) => (
          <div key={`b${i}`} className="flex items-center gap-1.5 h-4">
            <div className="w-[80px] text-right text-[13px] text-green-400/80 truncate shrink-0">
              {fmt(b.price)}
            </div>
            ...
            <div className="w-[48px] text-left text-[13px] text-green-400/50 shrink-0">
              {fmt(b.volume, 4)}
            </div>
          </div>
        ))}
