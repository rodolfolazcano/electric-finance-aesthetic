...
export function HedgePlainLanguagePanel({ result, availableCash, portfolioValorizado }: Props) {
  ...
  return (
    <div className="space-y-4 rounded-lg border border-border/40 bg-muted/5 p-5">
      ...
      plan.situacionSaldo.montoNecesarioDepositar > 0 ? (
        <div className="rounded-md border border-warning/40 bg-warning/10 p-3">
          <div className="flex items-center gap-2">
            ...
            <p className="font-mono text-[11px] text-warning">{plan.situacionSaldo.mensaje}</p>
          </div>
        </div>
      ) : (
        <div className="rounded-md border border-success/40 bg-success/10 p-3">
          <div className="flex items-center gap-2">
            <span className="text-sm"></span>
            <p className="font-mono text-[11px] text-success">{plan.situacionSaldo.mensaje}</p>
          </div>
        </div>
      )}
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`shrink-0 font-mono text-[10px] ${semaforo.color}`}>
                    ...
                    <div className="mt-3 space-y-2 border-t border-border/20 pt-2">
                      ...
                      {paso.advertencia && (
                        <div className="rounded border border-danger/30 bg-danger/5 p-2">
                          <p className="font-mono text-[10px] text-danger">{paso.advertencia}</p>
                        </div>
                      )}
                    </div>
                  )}
      ...
      <div className="rounded-md border border-border/20 bg-muted/10 p-3">
        <div className="flex items-center justify-between">
          <span className="font-mono text-[10px] text-muted-foreground">Costo estimado total</span>
