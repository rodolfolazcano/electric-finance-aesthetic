...
export function CalculadoraPresupuesto() {
  ...
  const removeGasto = useCallback(
    (i: number) => setGastos((prev) => prev.filter((_, idx) => idx !== i)),
    [],
  );
  const updateGasto = useCallback((i: number, field: keyof GastoRow, value: string | number) => {
    setGastos((prev) =>
      prev.map((g, idx) =>
        idx === i ? { ...g, [field]: field === "monto" ? Number(value) : value } : g,
      ),
    );
  }, []);
  ...
  const handleCalc = async () => {
    ...
    setLoading(true);
    const r = await fn({
      data: { ingresos, gastos: gastos.filter((g) => g.nombre.trim() && g.monto > 0) },
    });
    setResult(r);
    setLoading(false);
  ...
  const handleExport = useCallback(async () => {
    ...
    exportPresupuestoXLSX(
      { ingresos, gastos: gastos.filter((g) => g.nombre.trim() && g.monto > 0) },
      result!,
    );
          ...
          {gastos.map((g, i) => (
            <div key={i} className="flex gap-2 items-start">
              ...
              <button
                ...
                className="mt-1 text-muted-foreground hover:text-danger transition-colors"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div className="glass p-4 text-center">
                ...
                <div className="mono text-lg text-success">
                  ${result.totalIngresos.toLocaleString()}
                </div>
              </div>
              <div className="glass p-4 text-center">
                <div className="text-[10px] text-muted-foreground uppercase tracking-wider">
                ...
                <div
                  className={`mono text-lg ${result.alerta ? "text-danger" : "text-foreground"}`}
                >
                  ${result.totalGastos.toLocaleString()}
                </div>
              </div>
              <div className="glass p-4 text-center">
                <div className="text-[10px] text-muted-foreground uppercase tracking-wider">
                ...
                <div
                  className={`mono text-lg ${result.balance >= 0 ? "text-success" : "text-danger"}`}
                >
              ...
              <div className="glass p-4 text-center">
                ...
                <div
                  className={`mono text-lg ${result.tasaAhorro > 0 ? "text-success" : "text-danger"}`}
                >
                  {result.tasaAhorro}%
                </div>
              </div>
