...
export function CalculadoraPasivos() {
  ...
  const addDeuda = useCallback(
    () => setDeudas((prev) => [...prev, { nombre: "", monto: 0, tasa: 0, cuotaMinima: 0 }]),
    [],
  );
  const removeDeuda = useCallback(
    (i: number) => setDeudas((prev) => prev.filter((_, idx) => idx !== i)),
    [],
  );
  const updateDeuda = useCallback((i: number, field: keyof DeudaRow, value: string | number) => {
    setDeudas((prev) =>
      prev.map((d, idx) =>
        idx === i ? { ...d, [field]: field === "nombre" ? String(value) : Number(value) } : d,
      ),
    );
  }, []);
  ...
  const handleExport = useCallback(async () => {
    ...
    exportPasivosXLSX(
      { deudas: deudas.filter((d) => d.nombre.trim() && d.monto > 0), pagoMensual, estrategia },
      result!,
    );
    ...
    <div className="grid w-full grid-cols-1 gap-5 lg:grid-cols-[minmax(0,380px)_minmax(0,1fr)]">
      <div className="glass p-5 space-y-4">
        ...
        <div>
          ...
          <select
            value={estrategia}
            onChange={(e) => setEstrategia(e.target.value as "avalancha" | "bola-nieve")}
            className="w-full h-8 text-xs rounded-md border border-input bg-transparent px-3 mt-1"
          >
            <option value="avalancha">Avalancha (mayor tasa)</option>
            <option value="bola-nieve">Bola de nieve (mayor deuda)</option>
          </select>
        </div>
                ...
                <tbody>
                  {resumenDeudas.map((r, i) => (
                    <tr key={i} className="border-b border-border/30 hover:bg-muted/20">
                      ...
                      <td className="px-2 py-2 text-right">{r.meses}</td>
