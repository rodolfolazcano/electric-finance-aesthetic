...
export function MicrostructurePanel({ sigma0 = 1, sigmaU = 2, Vminus = 40, Vplus = 80, probInformed = 0.5, theta = 0.5 }: { sigma0?: number; sigmaU?: number; Vminus?: number; Vplus?: number; probInformed?: number; theta?: number }) {
  const lambda = kyleLambda(sigma0, sigmaU);
  const beta = kyleBeta(sigma0, sigmaU);
  const gm = glostenMilgrom({ Vminus, Vplus, probInformed, theta });
    ...
    <div className="rounded-xl border border-border/40 bg-card p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 font-mono text-sm">
        ...
        <div className="rounded border border-border/30 p-2">
          ...
          <div className="text-xs text-muted-foreground">spread {gm.spread.toFixed(2)} rel {(rel*100).toFixed(2)}% · P[V+|Buy]={gm.pPosteriorBuy.toFixed(2)}</div>
          <div className="text-xs text-muted-foreground mt-1">V-={Vminus} V+={Vplus}</div>
        </div>
      </div>
      <p className="text-xs text-muted-foreground">Bottom-up empírico vs EMT. Spread como proxy liquidez (Zoology). Bayes exacto con μ={probInformed.toFixed(2)} θ={theta.toFixed(2)}; caso canónico 0.5→P[V+|Buy]=0.75. No tradear si spread &gt;1%.</p>
    </div>
  );
}
