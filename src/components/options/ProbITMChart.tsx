...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-background/95 border border-border/60 rounded px-2 py-1.5 text-[9px] font-mono shadow-md max-w-[200px]">
      ...
      <p className="text-muted-foreground">
        Prob ITM: <span className="text-foreground">{(d.probITM * 100).toFixed(1)}%</span>
      </p>
      <p className="text-muted-foreground">
...
export function ProbITMChart({ options, spot }: ProbITMChartProps) {
  const { grupos } = useMemo(() => {
    const filtradas = options.filter((o) => o.probITM != null);
